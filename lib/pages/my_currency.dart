...
class _MyCurrencyState extends State<MyCurrency> {
                                ...
                                sliver: SliverList(
                                  delegate: SliverChildBuilderDelegate(
                                    (BuildContext context, int index) {
                                      ...
                                      debugPrint("te-->$intByValue}");
                                      debugPrint("te-->$intByValueOnChange");
  ...
  void _reorderDone(Key item) async {
    ...
    if (value.isNotEmpty && code.isNotEmpty) {
      int indexS = selectedList.indexWhere((element) => element.code == code);
      Utility.setIntPreference(Constants.indexByValueOnChange, indexS);
      debugPrint("I_mmm_Pr_$indexS");
      if (indexS != -1) {
        double v2 = double.parse(value);
        selectedList[indexS].controller.text =
            getFormatText(v2.toStringAsFixed(MyColors.decimalFormat));
      }
    }
